import {
  ...
  OffMessageListener,
  SealxHeader,
  MessageBeforeSendHook,
  MessageAfterSendHook,
} from '../contracts';
...
export default abstract class MessagerBase implements Messager {
  ...
  protected async receiveMessage(message: SealxRequest) {
    ...
    const allChannelAllTopicKey = this.topic(
      MessageChannel.ALL,
      SealxTopic.ALL,
    );
  ...
  addBeforeSendHook(hook: MessageBeforeSendHook) {
  ...
  addAfterSendHook(hook: MessageAfterSendHook) {
  ...
  protected async applyBeforeSendHooks(request: SealxRequest) {
    let next = request;
  ...
  protected async applyAfterSendHooks(response: SealxResponse) {
    let next = response;
  ...
  protected syncSessionFromResponse(response: SealxResponse) {
    if (response.session) {
   ...
   */
  reply: MessageReply = async <T = any>(
    message: T,
   ...
   * @note Subclasses must implement the actual message posting logic in postMessage()
   */
  replyError: MessageReply = async <T = any>(
    error: T,
    request: SealxRequest,
    end: boolean = true,
  ): Promise<SealxResponse<T>> => {
  ...
  send: MessageSend = async <T = any>(
    ...
    sendMsg = (await this.applyBeforeSendHooks(sendMsg)) as SealxRequest<T>;
    // if (chrome.tabs) {
    ...
    return new Promise((resolve, rejected) => {
      ...
      const handle = async (
        event: MessageEvent<SealxResponse> | SealxResponse,
      ) => {
  ...
  async *sendStream<T = any>(
     ...
     */
    const handle = async (event: MessageEvent<SealxResponse<T>>) => {
  ...
  on: MessageListener = (
    ...
    const handler1 = async (message: SealxRequest) => {
      ...
      if (
        (message.topic === topic || topic === SealxTopic.ALL) &&
    ...
    // 返回取消订阅函数
    return () => {
      this.handlers[topicKey] = this.handlers[topicKey].filter(
        (h) => h !== handler1,
      );
    };
  };
