...
export const slatGenerator = () => {
    const array = new Uint8Array(16); // 16 字节的随机值
    crypto.getRandomValues(array);
    return Array.from(array).map(byte => byte.toString(16).padStart(2, '0')).join('');
};
export const deriveKeyFromPin = async (pin, slat) => {
...
// pin码加密私钥
export const encryptPrivateKey = async (privateKey, pin, slat) => {
...
// pin码解密私钥
export const decryptPrivateKey = async (pin, encodePrivateKey, iv, slat) => {
    try {
        const key = await deriveKeyFromPin(pin, slat);
...
//# sourceMappingURL=cropto.js.map