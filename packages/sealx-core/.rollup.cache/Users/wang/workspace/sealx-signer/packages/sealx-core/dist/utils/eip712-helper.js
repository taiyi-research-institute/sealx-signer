 ...
 */
export function convertToISOFormat(timeValue) {
    if (!timeValue) {
        return '';
    }
    try {
 ...
 */
export const parseSignContent = (signContent) => {
 ...
 * )
 */
export const checkTemplateArgValid = (keyMap, template, parentKey = '') => {
 ...
 * )
 */
export const buildSignRenderContext = (keyMap, message) => {
    try {
        const context = {};
 ...
 */
export const layoutRender = (template, context) => {
    const render = _template(template);
    return render(context);
};
//# sourceMappingURL=eip712-helper.js.map