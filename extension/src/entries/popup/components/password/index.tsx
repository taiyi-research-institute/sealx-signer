...
export const Password = ({
  ...
  const retrySuccessTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const isActivationPending =
    requiresClickToType && !hasUserActivated && !displayPassword;
  const shouldAutoFocus = autoFocus && !isActivationPending;
  const chars = useMemo(
    () => displayPassword.padEnd(6, '').split(''),
    [displayPassword],
  );
  ...
  const setUserActivated = useCallback(
    (value: boolean) => {
      setActivationState({
        key: activationKey,
        activated: value,
  ...
  useEffect(() => {
    ...
    const enteredClickToTypeMode =
      requiresClickToType && !previousRequiresClickToTypeRef.current;
  ...
  useEffect(() => {
    ...
    return () => {
      console.log(
        '[Password:relay] unmounting — setting STOP via storage.session',
      );
  ...
  const scheduleStableReset = useCallback(() => {
    if (retrySuccessTimerRef.current)
      clearTimeout(retrySuccessTimerRef.current);
  ...
  const emitPasswordChange = useCallback(
    (value: string) => {
      if (readonly) return;
    ...
    return (
      ...
      >
        <button
          ...
          aria-hidden='true'
        >
          <div className='password-activation-content hover:cursor-pointer flex items-center justify-center'>
            <span className='password-activation-icon'>
              <span />
              <span />
              <span />
            </span>
            <span className=' text-[1.125rem]'>Click to enter PIN</span>
          </div>
        </button>
      </div>
    );
